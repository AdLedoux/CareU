...
class AIWeight(APIView):
    def post(self, request):
        ...
        if not logs or len(logs) < 3:
            return Response({"result": "More weight data is needed for analysis (at least 3 records)."}, status=status.HTTP_200_OK)
        weight_text_lines = []
        for w in logs:
...
class AIMood(APIView):
    def post(self, request):
        ...
        if not moods or len(moods) < 1:
            return Response({"result": "More mood data is needed for analysis (at least one record). "}, status=status.HTTP_200_OK)
        mood_lines = []
        for m in moods:
